...
spn = SPN()
spn.make_fast_model_from_file('../Modelz/' + dataset_name +'.spncc.spn.txt', random_weights=True, cont=False, classify=False)
spn.add_data('../Dataz/' + dataset_name + '.ts.data', 'train', True)
spn.add_data('../Dataz/' + dataset_name + '.test.data', 'test')
start = time.time()
spn.start_session()
spn.train(20, spn.data.train, minibatch_size=minibatch_size, count=step, gd=gd)#, ngd=2)
end = time.time() - start
